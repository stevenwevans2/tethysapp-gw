                ...
                if (feature.TsTime) {
                    layer.on({
                        click: function showResultsInDiv() {
                                ...
                                series: [
                                    {
                                        data: data,
                                        name:
                                            "Depth to Water Table (" + length_unit + ")",
                                        marker: { enabled: true },
                                        visible: (function() {
                                            type = $("#select_view")
                                                .find("option:selected")
                            ...
                            $.ajax({
                                url: apiServer + "get_timeseries/",
                                type: "GET",
                                data: {
                                    region: region,
                                    netcdf: $("#available_dates")
                                        .find("option:selected")
                                        .val(),
                                    hydroid: feature.properties.HydroID
                                },
                            ...
                            testTimeLayer._timeDimension.on(
                                "timeload",
                                function() {
                                    if (!mychart) {
                                        return
...
function confirm_delete() {
    ...
    var name = $("#available_dates")
        ...
        .val()
    var x = confirm(
        "Are you sure you want to delete the current NetCDF Raster? (" + name + ")"
    )
    if (x) {
        $.ajax({
